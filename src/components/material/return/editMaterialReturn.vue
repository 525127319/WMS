        ...
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item :to="{path:'/materialReturn'}">
                    <i class="el-icon-date"></i> 生产</el-breadcrumb-item>
                <el-breadcrumb-item>编辑退料单</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class='form-box'>
                <el-form ref="form" :model="form" :rules="rules" label-width="100px">
                    ...
                    <el-form-item label="收入仓库" prop="repertoryId">
                        ...
                        <el-col :span="10">
                        <el-select style="width: 100%" v-model="form.repertoryId" placeholder="请选择" @change="getRegionListById">
...
export default {
    ...
    data() {
        return {
            ...
            rules:{
                pickTime:[{type: 'date',required: true, message: "请选择退料时间", trigger: "blur"}],
                orderId:[{required: false,message:"请输入关联订单号",trigger:"blur"}],
                returnee:[{required:true,message:"请选择退货人",trigger:"change"}],
    ...
    methods:{
        onSubmit(formName,type){
            ...
            _this.$refs[formName].validate(valid => {
                if(valid){
                    ...
                    delete _this.form.list;
                    var params = {'returnMaterial':_this.form,'list':_this.returnMaterialList};
                    console.log(JSON.stringify(params));

                    updateReturnMaterials(params).then(res =>{
        ...
        findReturnMaterialsById(){
            var _this = this;
            var params = { 'id': _this.$route.query.materialReturnId };
            findReturnMaterialsById(params).then(res => {
                let { code, msg, data } = res;
                if (code == 200) {
                    _this.dialogTableVisible = true;
                    _this.form = data.returnMaterialsInfoVO;
                    _this.form.pickTime = new Date(_this.form.pickTime);

                    _this.returnMaterialList = data.returnMaterialsInfoVO.list;
                    _this.getRegionListById();
                } else if (code == 500) {
                    _this.$message.error("系统异常!");
                } else if (code == 400) {
       ...
       // 下拉框的值改变时触发,获取所有库位信息
        getRegionListById() {
            var _this = this;
            var params = { id: _this.form.repertoryId };
            getRegionListByRepertoryId(params).then(res => {
                ...
                if (code == 200) {
                    ...
                    //         region.id = _this.returnMaterialList[i].regionId;
                    //         region.name = _this.returnMaterialList[i].regionName;
                    //         var index = _this.regionList.indexOf(region);
                    //     }
                    // }
                } else if (code == 14000) {
                    _this.regionList = [];
