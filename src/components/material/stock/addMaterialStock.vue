    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>新增备料单</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form-box">
            <el-form ref="form" :rules="rules" :model="form" label-width="80px">
                <el-form-item label="备料时间">
                    <el-col :span="10">
                        <el-date-picker type="date" :editable="false" placeholder="选择日期" v-model="form.produceTime" style="width: 100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
                ...
                <!-- <el-form-item prop="remark" label="关联备料单号">
                    <el-col :span="10">
                        <el-input v-model="form.remark" @click.native="showDialogPrepares"></el-input>
                    </el-col>
                </el-form-item> -->
                ...
                <!--新增-->
                <el-dialog title="关联生产备料单列表" v-model="addTableVisiblePrepares" :close-on-click-modal="false">
                    <el-col :span="10">
                    ...
                    <el-table ref="productTable" :data="productTable" highlight-current-row @row-click="handleCurrentChange" style="width: 100%">
                        ...
                        <el-table-column property="orderDate" label="订单交期日期"></el-table-column>
                        <el-table-column property="orderDate" label="计划生产日期"></el-table-column>
                ...
                <el-table ref="materialDetailList" :data="materialDetailList" border highlight-current-row style="width: 100%">
                    ...
                    <!-- <el-table-column property="specification" label="计划备料数量"></el-table-column> -->
                ...
                <el-dialog title="物料列表" size="large" v-model="addTableVisible" :close-on-click-modal="false">
                    ...
                    <el-table ref="productTable" :data="productTable" highlight-current-row @row-click="handleCurrentChange" style="width: 100%">
                        ...
                        <!-- <el-table-column property="unitPriceExcludingTax" label="成本单价"></el-table-column> -->
...
import {
    addMaterialStock,//新增备料单
    findPrepareMaterialsCode,//自动生成备料单号
    findOrderForReady,//备料单列表
...
export default {
    ...
    data: function() {
        return {
            ...
            addTableVisiblePrepares: false,//关联生产备料弹框
    ...
    methods: {
        ...
        onSubmit(formName, type) {
            ...
            _this.$refs[formName].validate(function(valid) {
                if (valid) {
                    ...
                    if (!flag) {
                        addMaterialStock(params).then(res => {
                            let { code, msg, data } = res;
                            if (code == 200) {
                                _this.$message.success('新增备料单信息成功!');
                                _this.$router.push({ name: 'materialStock' })
                            } else if (code == 14001) {
                                _this.$message.error('新增备料单信息失败!');
                            } else if (code == 500) {
                                _this.$message.error('系统错误,新增备料单信息失败!');
                            }
                        })
                    }
                } else {
        ...
        queryPreparesList(){
            ...
            findOrderForReady(params).then(res => {
                let { code, msg, data } = res;
                if (code == 200) {
                    _this.productTable = data.list;
                    _this.total_size = data.page.totalRecord;
        ...
        showDialogPrepares() {
            this.addTableVisiblePrepares = true;
            this.queryPreparesList();
        },
        ...
        //自动生成备料料单号
        findPrepareMaterialsCode() {
            var _this = this;
            findPrepareMaterialsCode().then(res => {
                let { code, msg, data } = res;
                if (code == 200) {
                    _this.form.id = data.id;
                    _this.form.number = data.productionInStorageCode;
                } else if (code == 500) {
                    _this.$message.error('系统异常!');
                }
            });
